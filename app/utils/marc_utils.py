...
from app.utils.indexer_consts import FIELDS_TO_CHECK, FIELDS_TO_CHECK_FOR_OMNIS
...
def prepare_name_for_indexing(descriptor_name: str) -> str:
    ...
    if descriptor_name:
        ...
        descriptor_name = ''.join(
            char.replace(
                char,
                ' ') if (not char.isalnum() and char not in excluded_chars) else char for char in descriptor_name)
...
def get_terms_to_search_and_references_to_raw_flds(marc_record: Record,
                                                   for_omnis: bool = False) -> Dict[str, List[Field]]:
...
async def process_record(marc_record: Record,
                         identifier_type: str,
                         conn_auth_int,
                         conn_auth_ext,
                         polona: bool = False,
    ...
    The true heart and core of the entire service.
    """
    # get all terms to search for them in redis index and get all references to raw flds with these terms
