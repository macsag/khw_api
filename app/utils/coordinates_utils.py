...
def check_defg_034(coords: Field) -> bool:
    is_valid = True
    errors = []
...
def get_list_of_coords_from_valid_marc(coords: Field) -> list[float]:
    list_of_coords = []
...
def dms_to_decimal(single_coord: str) -> float:
    hemisphere = single_coord[0]
    d = single_coord[1:4]
    m = single_coord[4:6]
    s = single_coord[6:]
    sign = 1 if hemisphere in ['N', 'E'] else -1
    return float((int(d) + int(m) / 60 + int(s) / 3600) * sign)


def convert_to_bbox(coords: list[float]) -> str:
