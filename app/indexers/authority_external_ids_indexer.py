...
from clients.sqlite_clients.generic_client import GenericClient
...
class AuthorityExternalIdsIndex(object):
    ...
    def create_authority_external_ids_index(self) -> dict:
        ...
        if self.geonames:
            ...
            geo_geonames_cl = GenericClient(
                str(path_to_geo_geonames_db),
        ...
        if self.wikidata:
            ...
            geo_wikidata_cl = GenericClient(
                str(path_to_geo_wikidata_db),
            ...
            personal_wikidata_cl = GenericClient(
                str(path_to_personal_wikidata_db),
                'SELECT bn__descr_nlp_id, result__wkp_id, score, max_score FROM results',
                'wikidata_uri',
                id_processing_method=create_wikidata_uri)
            ...
            corporate_wikidata_cl = GenericClient(
                str(path_to_corporate_wikidata_db),
                'SELECT bn__descr_nlp_id, result__wkp_id, score, max_score FROM results',
                'wikidata_uri',
                id_processing_method=create_wikidata_uri)
            ...
            subject_wikidata_cl = GenericClient(
                str(path_to_subject_wikidata_db),
                'SELECT bn__descr_nlp_id, result__wkp_id, score, max_score FROM results',
                'wikidata_uri',
                id_processing_method=create_wikidata_uri)
            ...
            genre_wikidata_cl = GenericClient(
                str(path_to_genre_wikidata_db),
                'SELECT bn__descr_nlp_id, result__wkp_id, score, max_score FROM results',
                'wikidata_uri',
                id_processing_method=create_wikidata_uri)
        ...
        if self.orcid:
            ...
            personal_orcid_cl = GenericClient(
                str(path_to_personal_orcid_db),
    ...
    def index_in_redis(self):
        ...
        chunks = [dict(itertools.islice(
            keys_to_json.items(), i, i + chunk_max_size)) for i in range(0, len(keys_to_json), chunk_max_size)]
